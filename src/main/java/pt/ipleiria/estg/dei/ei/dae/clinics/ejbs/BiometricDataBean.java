...
public class BiometricDataBean {
    ...
    public BiometricData create(long biometricDataTypeId, double value, String notes, long patientId, long personId, String source, Date createdAt) throws MyEntityNotFoundException, MyIllegalArgumentException, MessagingException, MyUnauthorizedException {
        ...
        TypedQuery<BiometricData> query = entityManager.createQuery(
                "SELECT b FROM BiometricData b WHERE b.biometric_data_type.id = :type_id AND b.patient.id = :patient_id AND b.deleted_at IS NULL ORDER BY b.created_at DESC",
                BiometricData.class);
    ...
    public boolean delete(long id) throws MyEntityNotFoundException, MyIllegalArgumentException, MessagingException {
        ...
        query.setParameter("type_id", biometricDataType.getId()).setParameter("patient_id", patient.getId())
                .setLockMode(LockModeType.OPTIMISTIC);
        List<BiometricData> latestBioDatas = query.setMaxResults(1).getResultList();
        BiometricData latestBioData = latestBioDatas.size() == 0 ? null : latestBioDatas.get(0);
        ...
        if (latestBioData == null) return true;
        BiometricDataIssue biometricDataIssue = latestBioData.getBiometricDataIssue();
